...
# Content Enhancer block : tf version has a content enhancer block.  Original file description : First block (low-level feature extractor) 
...
class Disc_Transformer(nn.Module): # returns a pixel level classification
  def __init__(self, in_ch, out_ch, repeat, negative_slope= 0.01, stride=1, bias=False):
    ...
    for r in range(0, repeat):
        layers.append(
                Residual_block_Disc(ch_tmp, ch_tmp//2, stride=1, bias=False)
            )
        ch_tmp = ch_tmp // 2
...
class Efficient_Unet_disc(nn.Module):
  def __init__(self, in_ch, out_ch, negative_slope, filter_base=16, bias=False):
    ...
    #Encoder1
    self.down_0 = Residual_block_Disc(in_ch, f1, stride=1) #[B,C,W,H]-->[B,f1,W,H] 48

    #Encoder2
    self.down_1 = Residual_block_Disc(f1, f2, stride=2) #[B,f1,W,H]-->[B,f2,W/2,H/2] 64

    #Encoder3
    self.down_2 = Residual_block_Disc(f2, f3, stride=2) #[B,f2,W/2,H/2]-->[B,f3,W/4,H/4] 80

    #Bridge
    self.bridge = Residual_block_Disc(f3, fb, stride=2) #[B,f3,W/4,H/4]-->[B,fb,W/8,H/8] 96
