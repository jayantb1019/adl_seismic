...
class InterpretationDataModule(pl.LightningDataModule) : 
    ...
    def setup(self, stage) :
        if stage == 'fit' or None : 
            self.train_dataset = InterpretationDataset(dataset=np.load(self.train_data_path), labels = np.load(self.train_labels_path) ,
                                                   patch_info= pd.read_csv(self.train_patch_index_path) ,  
                                                   training_config= self.training_config, mode='train')
            
            self.val_dataset = InterpretationDataset(dataset=np.load(self.val_data_path), labels = np.load(self.val_labels_path) ,
                                                   patch_info= pd.read_csv(self.val_patch_index_path) ,  
                                                   training_config= self.training_config, mode='val')
            
        if stage == 'test' : 
            self.test_dataset = InterpretationDataset(dataset=np.load(self.test_data_path), labels = np.load(self.test_labels_path) ,
